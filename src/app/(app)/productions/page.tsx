...
type DataFilter = "all" | "rework" | "temp_only" | "final_done";

export default function ProductionsPage() {
  const { machines, productions, phases, assets, users } = useWorkspace();
  ...
  const filtered = useMemo(() => {
    return productions
      ...
      .filter((p) => {
        ...
        const myAssets = assets.filter((a) => a.productionId === p.id);
        if (myAssets.length === 0) return false;
        if (dataFilter === "rework")
          return myAssets.some((a) => a.reworkRequired);
        if (dataFilter === "temp_only")
          return myAssets.every((a) => a.dataKind === "temp");
        if (dataFilter === "final_done")
          return (
            myAssets.every((a) => a.dataKind === "final") &&
            myAssets.every((a) => !a.reworkRequired)
          );
        return true;
      })
      .sort(
  ...
  }, [productions, machineId, category, stateFilter, q, phaseHighlight, phases, assets, dataFilter]);
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs">
            ...
            <FilterRow
              ...
              options={[
                ...
                { value: "temp_only", label: "全て仮データ" },
                { value: "final_done", label: "本データ完了" },
              ]}
            />
          </div>
        </CardContent>
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        {filtered.map((p) => {
          ...
          const finalAssets = myAssets.filter(
            (a) => a.dataKind === "final"
          ).length;
          const reworkOpen = myAssets.filter((a) => a.reworkRequired).length;
          const finalRate =
            totalAssets > 0 ? Math.round((finalAssets / totalAssets) * 100) : 0;
          return (
            <Link key={p.id} href={`/productions/${p.id}`}>
              <Card
                className={`group h-full border-border/60 transition hover:border-primary/40 ${
                  reworkOpen > 0 ? "ring-1 ring-rose-500/30" : ""
                }`}
              >
                <CardContent className="space-y-3 p-4">
                  ...
                  {totalAssets > 0 && (
                    <div className="space-y-1.5">
                      ...
                      <Progress value={finalRate} className="h-1" />
                      {reworkOpen > 0 && (
                        <div className="inline-flex items-center gap-1 rounded-full bg-rose-500/10 px-2 py-0.5 text-[10px] font-semibold text-rose-300 ring-1 ring-rose-500/30">
